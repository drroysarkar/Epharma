...
const PaymentOut = () => {
 ...
 useEffect(() => {
  const fetchData = async () => {
    try {
      const purchaseData = await fetchPurchases(); // corrected
      setPurchases(purchaseData);
      setLoading(false);
    } catch (error) {
      toast.error('Failed to load payment out details');
      setPurchases([]);
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    filtered = filtered.filter((purchase) => {
      const purchaseDate = purchase.BillDate ? new Date(purchase.BillDate) : null;
      return purchaseDate && purchaseDate >= startDate && purchaseDate <= now;
    });
    ...
    if (filterText) {
      filtered = filtered.filter(
        (purchase) =>
          purchase.DistributorName?.toLowerCase().includes(filterText.toLowerCase()) ||
          purchase.BillNumber?.toLowerCase().includes(filterText.toLowerCase())
      );
    }

    setFilteredPurchases(filtered);
  }, [purchases, dateFilter, filterText]);

  // Calculate total paid amount
  const totalPaidAmount = filteredPurchases
    .reduce((sum, purchase) => sum + parseFloat(purchase.PaidAmount || 0), 0)
    .toFixed(2);
  ...
  const columns = [
    {
      ...
      selector: (row) => row.DistributorName || 'N/A',
      sortable: true,
      wrap: true,
    },
    {
    ...
    },
    {
      ...
      sortable: true,
      cell: (row) => `₹${parseFloat(row.PendingAmount || 0).toFixed(2)}`,
    },
    {
      name: 'Paid Amount',
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <main className="container mx-auto px-4 py-2">
        <div className="bg-white p-6 rounded-lg shadow-lg">
          {/* Header: Title, Search, Filter, and Total Paid Amount */}
          <div className="flex items-center mb-6">
            ...
            <div className="flex items-center space-x-4 ml-6" ref={searchRef}>
              {isSearchExpanded ? (
                <input
                  type="text"
                  placeholder="Search by Distributor Name or Bill Number..."
                  value={filterText}
            ...
            <div className="flex items-center bg-blue-50 border border-blue-200 rounded-lg px-4 py-2 shadow-sm ml-auto">
              ...
              <span className="text-lg font-semibold text-gray-700">
                Total Paid ({dateFilter}): ₹{totalPaidAmount}
              </span>
            </div>
          </div>
